...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 transcribe|preview --file /run/autostop-work-telegram/inbox/EXACT_FILE [--language ru] [--delete-after] | self-check" >&2
  exit 2
fi
...
if [[ "${self_check}" -eq 1 ]]; then
  [[ $# -eq 0 ]] || { echo "media_arguments_invalid=true" >&2; exit 2; }
else
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --file)
