...
def _partsapi_dry_run_calls(identifier: str, part_profile: dict[str, Any]) -> list[dict[str, Any]]:
    calls = [
        partsapi_catalog_lookup(operation="vin_decode_oe", identifier=identifier, dry_run=True),
    ]
...
def _oem_catalog_dry_run_call(identifier: str, item: dict[str, Any], requested_part: str) -> dict[str, Any]:
    context = _merged_item_context(item)
...
def _benchmark_status(summary: dict[str, Any]) -> str:
    ...
    if summary["high_identity_count"] == summary["count"] and summary["part_intent_recognized_count"] == summary["count"]:
...
def benchmark_vin_parts_lookup(
    ...
    *,
    ...
    for index, (item, identity) in enumerate(zip(items, identity_batch.get("results", [])), start=1):
        ...
        part_profile = normalize_part_intent(item_requested_part)
        ...
        partsapi_calls = _partsapi_dry_run_calls(identifier, part_profile) if include_partsapi_dry_run else []
        vin17_call = _vin17_dry_run_call(identifier) if include_vin17_dry_run else None
        oem_catalog_call = _oem_catalog_dry_run_call(identifier, item, item_requested_part) if include_oem_catalog_dry_run else None
