...
def sync_knowledge_base(store: ManagerMemoryStore | None = None) -> dict[str, Any]:
    ...
    with memory.connect() as conn:
        ...
        for domain, route in domains.items():
            ...
            optional_runtime_files = _unique_strings([str(item) for item in route.get("optional_runtime_files", [])])
            optional_status = {
...
def audit_knowledge_base(store: ManagerMemoryStore | None = None) -> dict[str, Any]:
    ...
    for domain, route in domains.items():
        ...
        for raw_path in _unique_strings([str(item) for item in route.get("optional_runtime_files", [])]):
...
def _optional_runtime_status(route: dict[str, Any]) -> dict[str, Any]:
    files = _unique_strings([str(item) for item in route.get("optional_runtime_files", [])])
