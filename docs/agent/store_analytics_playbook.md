...
- Public Gateway v2 surface remains exactly 24 tools; the hidden Manager raw
  registry has 64 tools.

The runtime needs `AUTOSTOP_STORE_API_URL` and a dedicated
`AUTOSTOP_STORE_READ_TOKEN`. Production accepts only
`https://autostop24.shop/internal/agent/v1`; HTTP is allowed only for loopback
tests. The client never follows redirects, so the bearer cannot cross an
origin boundary. Never print or persist the token outside runtime `.env`.
