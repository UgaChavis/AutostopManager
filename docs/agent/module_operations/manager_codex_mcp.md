...
| `scripts/sync_offline_parts_catalog_release.py` | `--verify-only` RO проверяет pinned release, originals/text/index и synthetic search без загрузки; без флага LOCAL скачивает закреплённый GitHub ZIP только при нехватке каталога, проверяет SHA/размер, импортирует и OCR до maintenance. Вызывается согласованным CRM deploy; при ошибке останавливает выпуск до обслуживания. |
