...
def test_read_plan_orders_vin_specific_part_lookup_after_identity_and_keeps_steps_read_only():
    request = CaseRequest(
        ...
        available_sources=(
            ...
            SourceDescriptor("brand_epc", SourceKind.OEM),
        ),
    ...
    oem_primary = next(step for step in plan.steps if step.step_id.endswith("oem_part:brand_epc:p0"))
    ...
    assert all(step.source_id in {"crm_context", "brand_epc"} for step in plan.steps)
...
def test_claim_is_blocked_when_its_identity_dependency_has_no_read_route():
    request = CaseRequest(
        ...
        available_sources=(SourceDescriptor("brand_epc", SourceKind.OEM),),
    )
